...
module.exports = (db) => {
  ...
  router.get("/:id", function (req, res) {
    Promise.all([
      ...
      providersDbHelper.getCategoriesForProvider(db, req.params.id),
    ])
      .then((result) => {
        console.log("THIS IS CAT FOR PROV", result);
        res.json(result);
      })
      .catch((err) => {
        console.log(err);
  ...
  router.get("/:id/newListings", function (req, res) {
    Promise.all([
      ...
      offersDbHelper.getOffersForProvider(db, req.params.id),
    ])
  ...
  router.get("/:id/jobsCompleted", function (req, res) {
    ...
    providersDbHelper
      .getCompletedJobs(db, req.params.id)
      .then((result) => {
        // console.log(result);
        res.json(result);
      })
  ...
  router.get("/:id/assignedJobs", function (req, res) {
    console.log("route:get assigned jobs");
    providersDbHelper
      .getUnfinishedAssignedJobs(db, req.params.id)
      .then((result) => {
        // console.log(result);
        res.json(result);
      })
  ...
  router.post("/:id/offer", function (req, res) {
    ...
    offersDbHelper
      .getFirstOfferByRequestIdAndProviderId(db, request_id, provider_id)
      .then((data) => {
        if (data) {
        ...
        Promise.all([
          offersDbHelper.addOfferForRequestByProvider(
            db,
            request_id,
            provider_id,
            quote,
          ...
          requestDbHelper.getClientForRequest(db, request_id),
        ])
          .then((data) => {
  ...
  router.put("/:id/assignedJobs/:jobId/update", function (req, res) {
    ...
    requestDbHelper
      .updateAssignedJob(db, req.params.jobId, req.body.date)
      .then((result) => {
