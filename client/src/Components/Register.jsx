...
export default function Register({ currentUser, setCurrentUser, categories }) {
  ...
  useEffect(() => {
    if (isRegistered) {
      if (currentUser.is_provider) {
        history.push(`/provider/${currentUser.id}`);
      } else {
              ...
              localCategories.map((category, index) => (
                <ButtonGroup toggle key={index}>
                  <ToggleButton
                    ...
                    onChange={(event) =>
                      handleButtonCheck(index, event.target.checked)
                    }
                  >
                    {" "}
                    {category.name}{" "}
